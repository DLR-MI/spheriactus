...
void ULidarPointCloud::BuildCollision(TFunction<void(bool)> CompletionCallback)
{
	...
	TSharedRef<FLidarPointCloudNotification, ESPMode::ThreadSafe> Notification = Notifications.Create("Building Collision", nullptr, "LidarPointCloudEditor.ToolkitCollision");
...
FBox ULidarPointCloud::CalculateBoundsFromPoints(const FLidarPointCloudPoint* Points, const int64& Count)
{
	...
	if (Points && Count > 0)
	{
		if (Count == 1) // Single point case
		{
			FVector PointLocation = (FVector)Points->Location;
			FVector Offset = PointLocation * 0.01f; // 1% of the point's location
			Offset = Offset.GetAbs(); // Ensure positive values for offset
			Bounds = FBox(PointLocation - Offset, PointLocation + Offset); // Small percentage-based box around the point
		}
		else  // Multiple points
		{
			...
			// Check if Bounds is still invalid (e.g., all points are identical, or Bounds is degenerate)
			if (!Bounds.IsValid || Bounds.GetExtent().IsZero())
			{
				// Use the location of the first point to calculate an offset
				FVector PointLocation = (FVector)Points->Location;
				FVector Offset = PointLocation * 0.01f; // 1% of the point's location
				Offset = Offset.GetAbs(); // Ensure positive values for offset
				Bounds = FBox(PointLocation - Offset, PointLocation + Offset); // Small percentage-based box around the point
