...
void FLidarPointCloudOctreeNode::InsertPoints_Dynamic(const FLidarPointCloudPoint* Points, const int64& Count, const FVector3f& Translation)
{
	...
	else
	{
		...
		{
			Data.Emplace(PointsPtr->Location + Translation, PointsPtr->Color, !!PointsPtr->bVisible, PointsPtr->ClassificationID, PointsPtr->Normal, PointsPtr->PointID);
		}
	}
}
		...
		if (FGridAllocation* GridCell = NewGridAllocationMap.Find(InGridData.Index))
		{
			...
			if (InGridData.DistanceFromCenter < GridCell->DistanceFromCenter)
			{
				if (bStoreInBucket)
				{
					...
					//UE_LOG(LogTemp, Error, TEXT("!!!!!!!!!!!!!!!!PointID: %d"), Other.PointID);
					PointBuckets[GridCell->ChildNodeLocation].Emplace(Other.Location + Translation, Other.Color, !!Other.bVisible, Other.ClassificationID, Other.Normal , Other.PointID);
				}
			...
			else if(bStoreInBucket)
			{
				...
				//UE_LOG(LogTemp, Error, TEXT("!!!!!!!!!!!!!!!!PointID: %d"), Other.PointID);
				PointBuckets[InGridData.ChildNodeLocation].Emplace(AdjustedLocation, Other.Color, !!Other.bVisible, Other.ClassificationID, Other.Normal, Other.PointID);
			}
		}
		else
		{
		...
		for (TPair<int32, FGridAllocation>& Element : NewGridAllocationMap)
		{
			...
			if (FGridAllocation* GridCell = CurrentGridAllocationMap.Find(GridIndex))
			{
				...
				if (Element.Value.DistanceFromCenter < GridCell->DistanceFromCenter)
				{
					...
					AllocatedPoint.PointID = Point.PointID;
					GridCell->DistanceFromCenter = Element.Value.DistanceFromCenter;
				}
				// ... otherwise add it straight to the bucket
				else if (bStoreInBucket)
				{
					//UE_LOG(LogTemp, Error, TEXT("!!!!!!!!!!!!!!!!PointID: %d"), Point.PointID);
					PointBuckets[Element.Value.ChildNodeLocation].Emplace(AdjustedLocation, Point.Color, !!Point.bVisible, Point.ClassificationID, Point.Normal, Point.PointID);
				}
			}
			else
			{
				//UE_LOG(LogTemp, Error, TEXT("!!!!!!!!!!!!!!!!PointID: %d"), Point.PointID);
				CurrentGridAllocationMap.Add(GridIndex, FGridAllocation(Data.Emplace(AdjustedLocation, Point.Color, !!Point.bVisible, Point.ClassificationID, Point.Normal, Point.PointID), Element.Value));
				NumPointsAdded++;
			}
		}
...
void FLidarPointCloudOctreeNode::InsertPoints_Dynamic(FLidarPointCloudPoint** Points, const int64& Count, const FVector3f& Translation)
{
	...
	else
	{
		for (FLidarPointCloudPoint** PointsPtr = Points, **DataEnd = PointsPtr + Count; PointsPtr != DataEnd; ++PointsPtr)
		{
			...
			Data.Emplace(PointData->Location + Translation, PointData->Color, !!PointData->bVisible, PointData->ClassificationID, PointData->Normal, PointData->PointID);
		}
	}
}
...
template <typename T>
void FLidarPointCloudOctree::GetPointsInSphere_Internal(TArray<FLidarPointCloudPoint*, T>& SelectedPoints, const FSphere& Sphere, const bool& bVisibleOnly, const FTransform* LocalToWorld) const
{
	SelectedPoints.Reset();
	if (LocalToWorld)
	{
		const FTransform3f Transform = (FTransform3f)*LocalToWorld;
		PROCESS_IN_SPHERE({ SelectedPoints.Add(Point->Transform(Transform)); });
	}
	else
	{
		PROCESS_IN_SPHERE({ SelectedPoints.Add(Point); });
...
void FLidarPointCloudOctree::GetPointsInSphere(TArray<FLidarPointCloudPoint>& SelectedPoints, const FSphere& Sphere, const bool& bVisibleOnly, const FTransform* LocalToWorld) const
{
	...
	{
		{
			const float RadiusSq = Radius * Radius; {
				TQueue< FLidarPointCloudOctreeNode*> Nodes; FLidarPointCloudOctreeNode* CurrentNode = nullptr; Nodes.Enqueue(Root); while (Nodes.Dequeue(CurrentNode)) {
					{
						{
							if (!bVisibleOnly || CurrentNode->NumVisiblePoints > 0) {
								if (Ray.Intersects(CurrentNode->GetBounds())) {
									if (!(bVisibleOnly && CurrentNode->NumVisiblePoints < CurrentNode->GetNumPoints())) {
										for (FLidarPointCloudPoint* Point = CurrentNode->GetPersistentData(), *DataEndPoint = Point + CurrentNode->GetNumPoints(); Point != DataEndPoint; ++Point) {
											if (Ray.Intersects(Point, RadiusSq)) {
												{
													return Point;
												}
											}
										...
										for (FLidarPointCloudPoint* Point = CurrentNode->GetPersistentData(), *DataEndPoint = Point + CurrentNode->GetNumPoints(); Point != DataEndPoint; ++Point) {
											if (Point->bVisible && Ray.Intersects(Point, RadiusSq)) {
												{
													return Point;
												}
											}
...
void FLidarPointCloudOctree::ApplyClassificationToPointsInSphere(const int32& ClassificationID, const FSphere& Sphere, const bool& bVisibleOnly)
{
	if (ClassificationID < 0 || ClassificationID > 31)
	{
		UE_LOG(LogLidarPointCloud, Warning, TEXT("Classification ID must be between 0 and 31. Value will be clamped."));
	}

	// Ensure the classification ID is within valid range (0-31)
	const uint8 ClampedClassification = FMath::Clamp(ClassificationID, 0, 31);
	PROCESS_IN_SPHERE_EX({ Point->ClassificationID = ClampedClassification; }, { CurrentNode->bRenderDataDirty = true; });
}

void FLidarPointCloudOctree::ApplyClassificationToAllPoints(const int32& ClassificationID, const bool& bVisibleOnly)
{
	if (ClassificationID < 0 || ClassificationID > 31)
	{
		UE_LOG(LogLidarPointCloud, Warning, TEXT("Classification ID must be between 0 and 31. Value will be clamped."));
	...
	// Ensure the classification ID is within valid range (0-31)
	const uint8 ClampedClassification = FMath::Clamp(ClassificationID, 0, 31);
	PROCESS_ALL_EX({ Point->ClassificationID = ClampedClassification; }, { CurrentNode->bRenderDataDirty = true; });
}
...
{
	// Apply a minimum threshold to each extent dimension
	FVector3f AdjustedExtent = InExtent;
	AdjustedExtent.X = InExtent.X <= 0.0f ? 0.01f : InExtent.X;
	AdjustedExtent.Y = InExtent.Y <= 0.0f ? 0.01f : InExtent.Y;
	AdjustedExtent.Z = InExtent.Z <= 0.0f ? 0.01f : InExtent.Z;

	// Log the adjustment for debugging purposes
	if (AdjustedExtent != InExtent)
	{
		PC_WARNING("Adjusted invalid bounds: Original: %s, Adjusted: %s", *InExtent.ToString(), *AdjustedExtent.ToString());
	}

	// Use AdjustedExtent instead of InExtent
	Extent = AdjustedExtent;
	const FVector3f UniformExtent = FVector3f(AdjustedExtent.GetMax());
