...
TSharedPtr<FLidarPointCloudImportSettings_ASCII> MakeImportSettings(const FString& Filename, const FVector2D& RGBRange, const FLidarPointCloudImportSettings_ASCII_Columns& Columns)
{
	...
	ImportSettings->SelectedColumns = {
		...
		FMath::Max(-1, Columns.ClassificationID)};
			...
			while (RemainingLoadedSize > 0 && !OutImportResults.IsCancelled())
			{
				...
				if (bEndOfLine)
				{
					...
					if (LoadedColumns > 1)
					{
						...
						const float ClassificationID = IsColumnPopulated[10] ? TempDoubles[10] : 0;

						OutImportResults.ClassificationsImported.AddUnique(ClassificationID);

						// If normals are assigned, use the appropriate constructor
						if (IsColumnPopulated[7] && IsColumnPopulated[8] && IsColumnPopulated[9])
						{
							OutImportResults.AddPoint(X, Y, Z, R, G, B, A, TempDoubles[7], TempDoubles[8], TempDoubles[9], uint32(LineIndex), uint8(ClassificationID));
						}
						// If ClassificationID/label_id exists in the data, we assign those labels
						else if (IsColumnPopulated[10])
						{
							OutImportResults.AddPoint(X, Y, Z, R, G, B, A, uint32(LineIndex), uint8(ClassificationID));
						}
						else
						{
							OutImportResults.AddPoint(X, Y, Z, R, G, B, A, uint32(LineIndex));
						}
					}
...
{
	// Initialize all 11 columns as -1 (none)
	...
	if (Reader)
	{
		...
		if (TotalSize > 0)
		{
			...
			for (int32 i = 0; i < Columns.Num(); i++)
			{
				...
				else if (ColName.Equals("label_id", ESearchCase::IgnoreCase) || ColName.Contains("classification"))
				{
					SelectedColumns[10] = i;
				}
				else
				{
					bNamedColumn = false;
			...
			if (!bNamedColumns)
			{
				...
				if (Columns.Num() >= 3)
				{
					...
					// Assume we are reading from a file previously exported by the Labelling Tool (XYZ RGB LABEL_ID LABEL_NAME POINT_ID)
					else if (Columns.Num() == 9)
					{
						SelectedColumns[3] = 3; // R
						SelectedColumns[4] = 4; // G
						SelectedColumns[5] = 5; // B
						SelectedColumns[10] = 6; // LABEL_ID or ClassificationID internally
					}
					// Assume XYZ RGB
