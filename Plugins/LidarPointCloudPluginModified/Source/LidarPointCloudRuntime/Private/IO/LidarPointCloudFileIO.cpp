...
void FLidarPointCloudImportResults::AddPoint(const float& X, const float& Y, const float& Z, const float& R, const float& G, const float& B, const float& A /*= 1.0f*/, const uint32& PointID, const uint8& ClassificationID)
{
	//UE_LOG(LogTemp, Error, TEXT("PointID: %d"), PointID);
	Points.Emplace(X, Y, Z, R, G, B, A, PointID, ClassificationID);
	Bounds += (FVector)Points.Last().Location;
	IncrementProgressCounter(1);
}
...
void FLidarPointCloudImportResults::AddPoint(const float& X, const float& Y, const float& Z, const float& R, const float& G, const float& B, const float& A, const float& NX, const float& NY, const float& NZ, const uint32& PointID, const uint8& ClassificationID)
{
	//UE_LOG(LogTemp, Error, TEXT("PointID: %d"), PointID);
	Points.Emplace(X, Y, Z, R, G, B, A, NX, NY, NZ, PointID, ClassificationID);
	Bounds += (FVector)Points.Last().Location;
	IncrementProgressCounter(1);
}
