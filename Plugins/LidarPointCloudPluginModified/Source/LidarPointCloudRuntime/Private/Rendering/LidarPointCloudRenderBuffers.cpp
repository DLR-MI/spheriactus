...
void FLidarPointCloudRenderBuffer::Resize(const uint32& RequestedCapacity)
{
	...
	// UE_LOG(LogTemp, Warning, TEXT("RequestedCapacity: %d"), RequestedCapacity);
	if (Capacity != RequestedCapacity)
	{
		ReleaseResource();
		Capacity = RequestedCapacity;
...
void FLidarPointCloudRenderBuffer::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	const uint32 PointSize = sizeof(FLidarPointCloudPoint) - sizeof(uint32);

	// Log the memory layout
	// UE_LOG(LogTemp, Warning, TEXT("InitRHI Buffer Layout:"));
	//UE_LOG(LogTemp, Warning, TEXT("- Point Size: %d"), PointSize);
	//UE_LOG(LogTemp, Warning, TEXT("- Location Offset: %d"), offsetof(FLidarPointCloudPoint, Location));
	//UE_LOG(LogTemp, Warning, TEXT("- Color Offset: %d"), offsetof(FLidarPointCloudPoint, Color));
	//UE_LOG(LogTemp, Warning, TEXT("- Normal Offset: %d"), offsetof(FLidarPointCloudPoint, Normal));
	//UE_LOG(LogTemp, Warning, TEXT("- PointID Offset: %d"), offsetof(FLidarPointCloudPoint, PointID));
...
{
	// Calculate the total buffer size needed based on the actual structure size
	const size_t TotalBufferSize = NumPoints * (sizeof(FLidarPointCloudPoint) - sizeof(uint32));

	//UE_LOG(LogTemp, Warning, TEXT("Buffer Initialize:"));
	//UE_LOG(LogTemp, Warning, TEXT("- NumPoints: %d"), NumPoints);
	//UE_LOG(LogTemp, Warning, TEXT("- Point Size: %zu"), sizeof(FLidarPointCloudPoint));
	//UE_LOG(LogTemp, Warning, TEXT("- Total Buffer Size: %zu"), TotalBufferSize);
	//UE_LOG(LogTemp, Warning, TEXT("- Buffer Capacity: %d"), Capacity);

	// Resize the buffer to accommodate the full size
	//Resize(TotalBufferSize);
	Resize(NumPoints * 5);

	FRHICommandListBase& RHICmdList = FRHICommandListImmediate::Get();

	uint8* StructuredBuffer = (uint8*)RHICmdList.LockBuffer(Buffer, 0, NumPoints * (sizeof(FLidarPointCloudPoint) - sizeof(uint32)), RLM_WriteOnly);
	for (FLidarPointCloudPoint* P = Data, *DataEnd = P + NumPoints; P != DataEnd; ++P)
	{
		FMemory::Memcpy(StructuredBuffer, P, sizeof(FLidarPointCloudPoint) - sizeof(uint32));
		StructuredBuffer += (sizeof(FLidarPointCloudPoint) - sizeof(uint32));
	}
...
void FLidarPointCloudVertexFactory::FPointCloudVertexBuffer::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	VertexBufferRHI = RHICmdList.CreateBuffer(BufferSize, BUF_ShaderResource | BUF_Static | BUF_VertexBuffer, (sizeof(FLidarPointCloudPoint) - sizeof(uint32)), ERHIAccess::VertexOrIndexBuffer, CreateInfo);
	...
	{
		FMemory::Memcpy(Dest, Data, (sizeof(FLidarPointCloudPoint) - sizeof(uint32))); Dest++;
		FMemory::Memcpy(Dest, Data, (sizeof(FLidarPointCloudPoint) - sizeof(uint32))); Dest++;
		FMemory::Memcpy(Dest, Data, (sizeof(FLidarPointCloudPoint) - sizeof(uint32))); Dest++;
		FMemory::Memcpy(Dest, Data, (sizeof(FLidarPointCloudPoint) - sizeof(uint32))); Dest++;
	}
...
void FLidarPointCloudVertexFactory::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	//Elements.Add(AccessStreamComponent(FVertexStreamComponent(&VertexBuffer, 20, sizeof(FLidarPointCloudPoint), VET_UInt), 3));
...
void FLidarPointCloudSharedVertexFactory::InitRHI(FRHICommandListBase& RHICmdList)
{
	...
	//Elements.Add(AccessStreamComponent(FVertexStreamComponent(&VertexBuffer, 0, 0, VET_UInt), 3));
