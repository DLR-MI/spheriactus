...
FLidarPointCloudLODManager::FLidarPointCloudLODManager()
	...
	, CachedTotalPointCount(0)
	, CachedLastPointBudget(0)
	, CachedTotalPointsSelected(0)
{
}
...
void FLidarPointCloudLODManager::Tick(float DeltaTime)
{
	...
	CachedLastPointBudget = LastPointBudget;
...
int64 FLidarPointCloudLODManager::ProcessLOD(const TArray<FLidarPointCloudLODManager::FRegisteredProxy>& InRegisteredProxies, const float CurrentTime, const uint32 PointBudget, const TArray<FLidarPointCloudClippingVolumeParams>& ClippingVolumes)
{
	...
	// Node selection
	{
		...
		CachedTotalPointsSelected = TotalPointsSelected;
	}
...
void FLidarPointCloudLODManager::PrepareProxies()
{
	...
	CachedTotalPointCount = TotalPointCount;
}
...
void FLidarPointCloudLODManager::InitializeLogging()
{
	FString LogPathPerformance = FPaths::ProjectDir() + TEXT("PerformanceLog.csv");
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
	LogFileHandle = PlatformFile.OpenWrite(*LogPathPerformance, true, true);

	if (LogFileHandle)
	{
		FString Header = TEXT("Timestamp,TotalPointCount,PointBudget,VisiblePoints,FPS\n");
		LogFileHandle->Write((const uint8*)TCHAR_TO_ANSI(*Header), Header.Len());
	}

	// Start logging every second
	if (GEngine && GEngine->GameViewport && GEngine->GameViewport->GetWorld())
	{
		GEngine->GameViewport->GetWorld()->GetTimerManager().SetTimer(TimerHandle, [this]() { LogPerformanceMetrics(); },1.0f,true);
		UE_LOG(LogTemp, Warning, TEXT("Start logging every second"));
	}
}

void FLidarPointCloudLODManager::LogPerformanceMetrics()
{
	if (!LogFileHandle) return;

	UE_LOG(LogTemp, Warning, TEXT("Logging Performance Metrics"));
	...
	FString LogEntry = FString::Printf(TEXT("%s,%d,%d,%d,%.2f\n"),
		*TimeStamp,
		CachedTotalPointCount,
		CachedLastPointBudget,
		CachedTotalPointsSelected,
		FPS
	);

	LogFileHandle->Write((const uint8*)TCHAR_TO_ANSI(*LogEntry), LogEntry.Len());
	UE_LOG(LogTemp, Warning, TEXT("Entry added"));
