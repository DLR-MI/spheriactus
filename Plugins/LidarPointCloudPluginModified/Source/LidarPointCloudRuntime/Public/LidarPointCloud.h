...
public:
	...
	void ApplyClassificationToPointsInSphere(const int32& ClassificationID, FSphere Sphere, const bool& bVisibleOnly)
	{
		Sphere.Center -= LocationOffset;
		Octree.ApplyClassificationToPointsInSphere(ClassificationID, Sphere, bVisibleOnly);
	}

	/** Applies the given classification to all points */
	UFUNCTION(BlueprintCallable, Category = "Lidar Point Cloud")
	void ApplyClassificationToAllPoints(const int32& ClassificationID, const bool& bVisibleOnly) { Octree.ApplyClassificationToAllPoints(ClassificationID, bVisibleOnly); }
