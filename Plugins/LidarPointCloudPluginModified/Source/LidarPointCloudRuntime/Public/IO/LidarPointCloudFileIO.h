...
public:
	...
	FORCEINLINE void AddPoint(const FVector& Location, const float& R, const float& G, const float& B, const float& A = 1.0f, const uint32& PointID = 0, const uint8& ClassificationID = 0)
	{
		AddPoint(Location.X, Location.Y, Location.Z, R, G, B, A, PointID, ClassificationID);
	}

	void AddPoint(const float& X, const float& Y, const float& Z, const float& R, const float& G, const float& B, const float& A = 1.0f);

	void AddPoint(const float& X, const float& Y, const float& Z, const float& R, const float& G, const float& B, const float& A = 1.0f, const uint32& PointID = 0, const uint8& ClassificationID = 0);
	...
	void AddPoint(const float& X, const float& Y, const float& Z, const float& R, const float& G, const float& B, const float& A, const float& NX, const float& NY, const float& NZ, const uint32& PointID, const uint8& ClassificationID);
