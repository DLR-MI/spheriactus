...
public:
	...
	TArray<FLidarPointCloudPoint> GetPointsInSphere(FVector Center, float Radius, bool bVisibleOnly, bool bReturnWorldSpace)
	{
		TArray<FLidarPointCloudPoint> Points;
		GetPointsInSphere(Points, FSphere(Center, Radius), bVisibleOnly, bReturnWorldSpace);
		return Points;
	}
	void GetPointsInSphere(TArray<FLidarPointCloudPoint>& SelectedPoints, const FSphere& Sphere, const bool& bVisibleOnly, const bool& bReturnWorldSpace)
	{
		if (PointCloud)
		{
			FTransform LocalToWorld = GetLocalToWorld();
			PointCloud->Octree.GetPointsInSphere(SelectedPoints, Sphere.TransformBy(LocalToWorld.Inverse()), bVisibleOnly, bReturnWorldSpace ? &LocalToWorld : nullptr);
		}
	}
	...
	/** Applies the given classification to all points within the sphere*/
	UFUNCTION(BlueprintCallable, Category = "Lidar Point Cloud")
	void ApplyClassificationToPointsInSphere(int32 ClassificationID, FVector Center, float Radius, bool bVisibleOnly) { ApplyClassificationToPointsInSphere(ClassificationID, FSphere(Center, Radius), bVisibleOnly); }
	void ApplyClassificationToPointsInSphere(const int32& ClassificationID, const FSphere& Sphere, const bool& bVisibleOnly)
	{
		if (PointCloud)
		{
			PointCloud->ApplyClassificationToPointsInSphere(ClassificationID, Sphere.TransformBy(GetComponentTransform().Inverse()), bVisibleOnly);
		}
	}
